...
const regionCenter: Record<HotZone['region'], { cx: number; cy: number }> = {
  forehead: { cx: CX, cy: 75 },
  left_cheek: { cx: 85, cy: 200 },
  right_cheek: { cx: 215, cy: 200 },
  nose: { cx: CX, cy: 185 },
  chin: { cx: CX, cy: 310 },
  jaw: { cx: CX, cy: 280 },
};
...
const V: [number, number][] = [
  // ---- CONTOUR (outer face silhouette, 0-20) ----
...
function buildEdges(tris: [number, number, number][]): [number, number][] {
  ...
  for (const [a, b, c] of tris) {
    for (const [p, q] of [[a,b],[b,c],[a,c]] as [number,number][]) {
      ...
      if (!set.has(key)) { set.add(key); result.push([p, q]); }
    }
  }
  return result;
}
